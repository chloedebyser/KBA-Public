...
#### Processing ####
ProposalForm_to_KBAEBAR <- function(formName, dbName, KBASiteID){
    ...
    KBASite <- st_read(dsn=paste0(gdbsPath, dbName, ".gdb"), layer="KBASite") %>%
    ...
    # ** SAVE TO GEODATABASE **
    st_write(KBASite, dsn=paste0(gdbsPath, dbName, ".gpkg"), layer="KBASite")
    output <- "SUCCESS"
